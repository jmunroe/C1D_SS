MODULE trcnxt
   ...
   USE tranxt
   USE iom
   USE p4zriv, only: fraser_turb
   USE p4zsbc, only: ln_turb
# if defined key_bdy
...
CONTAINS
   ...
   SUBROUTINE trc_nxt( kt )
...
#endif
      IF (ln_turb) CALL  fraser_turb( kt )           ! set tracer condition in Fraser River
      ...
      !! Write out full phys trend before asselin filter and negative correction
      !IF( iom_use( "PHYSTRNO3" )) THEN
      !   CALL iom_put("PHYSTRNO3",(tra(:,:,:,jpno3)-trb(:,:,:,jpno3))*cvol(:,:,:)/r2dt(1))
      !ENDIF
...
#if defined key_trdtrc 
      IF( iom_use( "AFILTNO3" )) THEN ! save value before phys call
          z3d(:,:,:)=trn(:,:,:,jpno3)
      ENDIF
#endif
...
#if defined key_trdtrc 
      IF( iom_use( "AFILTNO3" )) THEN ! save value before phys call
         CALL iom_put( "AFILTNO3", (trb(:,:,:,jpno3)-z3d(:,:,:))*cvol(:,:,:)/r2dt(1))
      ENDIF
#endif
      ! trends computation

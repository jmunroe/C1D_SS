MODULE zdfgls
   ...
   REAL(wp) ::   rn_wvage_max	   ! Maximum "wave age" c_p/ustar (if nn_z0_met=2)
   REAL(wp) ::   rn_ustar_ref	   ! Fetch/duration wave growth limitation threshold on ustar [m/s] (if nn_z0_met=2)
   REAL(wp) ::   rn_frac_hs        ! Fraction of wave height as surface roughness (if nn_z0_met=2)
   ...
   REAL(wp) ::   rc02, rc02r, rc04                          ! coefficients deduced from above parameters
   REAL(wp), PUBLIC :: rc03                                 ! make this one public so we can calculate dissipation elsewhere
...
!CDIR NOVERRCHK         
      ...
      CASE ( 2 )             ! Roughness formulae according to Rascle et al., Ocean Modelling (2008)
         ! BMM: Added parameters rn_wvage_max and rn_ustar_ref to namelist_ref for tuning against WW3
         zdep(:,:)  = rn_wvage_max*TANH(2.*rn_ustar_ref/(28.*SQRT(MAX(ustars2(:,:),rsmall))))             ! Wave age (eq. 10)
      ...
      END DO
      ! output en and eps:
      IF( iom_use( "GLS_k" ) )  CALL iom_put( "GLS_k",en(:,:,:))
      IF( iom_use( "GLS_eps" ) )  CALL iom_put( "GLS_eps",eps(:,:,:))
      IF( iom_use( "GLS_G" ) )  CALL iom_put( "GLS_G", - avt(:,:,:) * rn2(:,:,:))
      !
      ...
      DO jk = 2, jpkm1
         ...
         END DO
      END DO
      IF( iom_use( "GLS_P" ) )  CALL iom_put( "GLS_P", shear(:,:,:))
      !
      z_elem_b(:,:,jpk) = 1._wp
      !
...
!CDIR NOVERRCHK
   ...
   SUBROUTINE zdf_gls_init
      ...
      NAMELIST/namzdf_gls/rn_emin, rn_epsmin, ln_length_lim,  &
         ...
         &            rn_crban, rn_charn,                     &
         &            rn_wvage_max, rn_ustar_ref, rn_frac_hs, &
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namzdf_gls in configuration namelist', lwp )
      ...
      IF(lwp) THEN                     !* Control print
         ...
         WRITE(numout,*) '      Maximum "wave age" (used if nn_z0_met=2)      rn_wvage_max   = ', rn_wvage_max
         WRITE(numout,*) '      Ustar fetch lim. thresh. [m/s] (nn_z0_met=2)  rn_ustar_ref   = ', rn_ustar_ref
         ...
         WRITE(numout,*) '      Minimum surface roughness (m)                 rn_hsro        = ', rn_hsro
      ...
      rsbc_zs2  = rn_frac_hs / grav * 665._wp                  ! Rascle formula for surface roughness (BMM: merged 0.85 in denominator into rn_frac_hs)

...
CONTAINS
   ...
   SUBROUTINE trc_stp( kt )
      ...
      REAL(wp), POINTER, DIMENSION(:,:,:) ::  cvol_b, cvol_a      ! 3D workspace
      REAL(wp), POINTER, DIMENSION(:,:,:,:) :: z4d, z4a
      REAL(wp)              ::  ztrai
      CHARACTER (len=25)    ::  charout 
      CHARACTER (len=20)  :: strsms

      CALL wrk_alloc( jpi , jpj, jpk , cvol_b, cvol_a )
      IF( l_trdtrc ) THEN ! save value before phys call
         CALL wrk_alloc( jpi , jpj, jpk , jptra,  z4d, z4a )
      ENDIF
      !!-------------------------------------------------------------------
      !
      ...
      IF( lk_vvl ) THEN                                                   ! update ocean volume due to ssh temporal evolution
         DO jk = 1, jpk
            ...
            cvol_b(:,:,jk) = e1e2t(:,:) * fse3t_b(:,:,jk) * tmask(:,:,jk)
            cvol_a(:,:,jk) = e1e2t(:,:) * fse3t_a(:,:,jk) * tmask(:,:,jk)
         END DO
      ...
      IF( MOD( kt , nn_dttrc ) == 0 ) THEN      ! only every nn_dttrc time step
         ...
         ELSE ;  CALL ctl_stop('lk_iomput required') !CALL trc_dia  ( kt )   ! output of passive tracers with old I/O manager
         ENDIF   
                                   CALL trc_sms      ( kt )       ! tracers: sinks and sources
         IF( l_trdtrc ) THEN ! save value before phys call
                 z4d(:,:,:,:)=trb(:,:,:,:)*SPREAD(cvol_b(:,:,:),4,jptra)
                 z4a(:,:,:,:)=tra(:,:,:,:)*SPREAD(cvol(:,:,:),4,jptra)
         ENDIF
                                   CALL trc_trp      ( kt )       ! transport of passive tracers
         IF( l_trdtrc ) THEN !
             strsms="PHS_" ! note: does not include bio sinking
             DO jn = jp_pcs0, jp_pcs1
                CALL trd_trc_novol( (trn(:,:,:,jn)*cvol_a(:,:,:)-z4d(:,:,:,jn))/rfact-z4a(:,:,:,jn),jn,strsms,kt )
             END DO 
             CALL wrk_dealloc(jpi,jpj,jpk,jptra, z4d, z4a)
         ENDIF
...
9300  FORMAT(i10,e18.10)
      !
      CALL wrk_dealloc( jpi , jpj, jpk , cvol_b, cvol_a )

MODULE trc
...
#if defined key_bdy
   USE bdy_oce, only: jp_bdy, nb_bdy, OBC_DATA
#endif
   ...
   LOGICAL             , PUBLIC                                    ::  lrst_trc = .FALSE. !: logical to control the trc restart write
...
!$AGRIF_DO_NOT_TREAT
   TYPE(OBC_DATA),    PUBLIC, DIMENSION(jptra,jp_bdy), TARGET ::  trcdta_bdy           !: bdy external data (local process)

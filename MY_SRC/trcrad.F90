...
CONTAINS
   ...
   SUBROUTINE trc_rad( kt )
      ...
      !!
      !! ** Method  : - SMELT / PISCES / LOBSTER: Set negative concentrations to zero
      ...
      IF( lk_pisces  )   CALL trc_rad_sms( kt, trb, trn, jp_pcs0 , jp_pcs1 )  ! PISCES model
   ...
   SUBROUTINE trc_rad_sms( kt, ptrb, ptrn, jp_sms0, jp_sms1, cpreserv )
      ...
      REAL(wp), POINTER, DIMENSION(:,:,:) ::   ztrtrdbE, ztrtrdnE   ! workspace arrays
      REAL(wp) :: zs2rdt
      ...
      CALL wrk_alloc( jpi, jpj, jpk, ztrtrdbE, ztrtrdnE )
      ...
      ELSE  ! total CFC content is not strictly preserved
         ...
         DO jn = jp_sms0, jp_sms1  
           ...
           IF( l_trdtrc ) THEN
              ...
              ztrtrdn(:,:,:) = ptrn(:,:,:,jn)                        ! save input trn for trend computation
           ENDIF
           ztrtrdbE(:,:,:) = ptrb(:,:,:,jn)                        ! save input trb for trend computation
           ztrtrdnE(:,:,:) = ptrn(:,:,:,jn)                        ! save input trn for trend computation
            ...
            IF( jn==jpno3) THEN
               IF( kt == nittrc000 ) THEN
                  zs2rdt = 1. / (  rdt * FLOAT( nn_dttrc ) )
               ELSE
                  zs2rdt = 1. / ( 2. * rdt * FLOAT( nn_dttrc ) )
               ENDIF
               ztrtrdbE(:,:,:) = ( ptrb(:,:,:,jn) - ztrtrdbE(:,:,:) ) * zs2rdt
               ztrtrdnE(:,:,:) = ( ptrn(:,:,:,jn) - ztrtrdnE(:,:,:) ) * zs2rdt
               CALL iom_put("NO3RAD",(ztrtrdbE+ztrtrdnE)*cvol(:,:,:)*tmask(:,:,:)* zs2rdt)
            ENDIF
            !
         ENDDO
      ...
      CALL wrk_dealloc( jpi, jpj, jpk, ztrtrdbE, ztrtrdnE )

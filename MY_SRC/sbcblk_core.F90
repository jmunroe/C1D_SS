...
902      IF( ios /= 0 ) CALL ctl_nam ( ios , 'namsbc_core in configuration namelist', lwp )
   ...
   SUBROUTINE blk_oce_core( kt, sf, pst, pu, pv )
      ...
      ENDIF
      qsr(:,:) = MAX(qsr(:,:), 0._wp)  ! as solar flux is found by differencing, force it non-negative
      ...
      ! ----------------------------------------------------------------------------- !
      !
      ! set tprecip and sprecip here (for output totol and solid precip in [k/m2/s]
      tprecip(:,:) = MAX(sf(jp_prec)%fnow(:,:,1), 0._wp) * rn_pfac                     ! force precip non negative
      sprecip(:,:) = MAX(sf(jp_snow)%fnow(:,:,1), 0._wp) * rn_pfac                     ! force snow non negative
      ...
      qns(:,:) = zqlw(:,:) - zqsb(:,:) - zqla(:,:)                                &   ! Downward Non Solar 
         ...
         &     + ( tprecip(:,:) - sprecip(:,:) )                                  &   ! add liquid precip heat content at Tair
         &     * ( sf(jp_tair)%fnow(:,:,1) - rt0 ) * rcp                          &
         &     + sprecip(:,:)                                                     &   ! add solid  precip heat content at min(Tair,Tsnow)

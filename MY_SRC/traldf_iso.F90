...
#if  defined key_dian
         IF( cdtype == 'TRC' .AND. jn == jpno3) THEN
            IF (iom_use("ULDFNO3")) THEN
               CALL iom_put( "ULDFNO3", -zftu(:,:,:))
            ENDIF
            IF (iom_use("VLDFNO3")) THEN
               CALL iom_put( "VLDFNO3", -zftv(:,:,:))
            ENDIF
         ENDIF
#endif
#if defined key_trdtrc
         IF( cdtype == 'TRC' .AND. l_trdtrc) THEN
            CALL  trd_flx( -zftu(:,:,:), jn, "DTX_","U" )
            CALL  trd_flx( -zftv(:,:,:), jn, "DTY_","V" )
         ENDIF
#endif 
...
#if  defined key_dian
         IF( cdtype == 'TRC' .AND. jn == jpno3) THEN
            IF (iom_use("WLDFNO3")) THEN
               CALL iom_put( "WLDFNO3", -ztfw(:,:,:))
            ENDIF
         ENDIF
#endif
#if defined key_trdtrc
         IF( cdtype == 'TRC' .AND. l_trdtrc) THEN
            CALL  trd_flx( -zftu(:,:,:), jn, "WLDF","W" )
         ENDIF
#endif

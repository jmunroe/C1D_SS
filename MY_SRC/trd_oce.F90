MODULE trd_oce
   ...
   USE par_trc                 ! (EO) moved from trdtrc_oce to simplify
...
# if ( defined key_trdtrc && defined key_iomput ) 
   ...
   LOGICAL, PUBLIC, DIMENSION(jptra) ::   ln_trdtrc = .FALSE.   !: large trends diagnostic to write or not (namelist)
# else
   LOGICAL , PUBLIC ::   l_trdtrc = .FALSE.       !: tracers  trend flag
# endif
   LOGICAL, PARAMETER ::   lk_trdmxl_trc = .FALSE.   !: ML trend flag (option removed in SMELT)

MODULE traadv_tvd
   ...
   PUBLIC   tra_adv_tvd_zts_bio    ! routine called by traadv.F90
...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: traadv_tvd.F90 6770 2016-07-01 11:43:36Z mathiot $
...
CONTAINS
   ...
   SUBROUTINE tra_adv_tvd_zts ( kt, kit000, cdtype, p2dt, pun, pvn, pwn,      &
      ...
      REAL(wp), DIMENSION(jpi,jpj,jpk,kjpt), INTENT(in   ) ::   ptb, ptn        ! before and now tracer fields
      REAL(wp), DIMENSION(jpi,jpj,jpk,kjpt), INTENT(inout) ::   pta             ! tracer trend 
      INTEGER                              , INTENT(in   ) ::   jnzts           ! number of sub-timesteps for vertical advection
      !
...
#if defined key_trdtrc            
            IF( ( cdtype == 'TRC') ) THEN
               CALL  trd_flx( ztrdx, jn, "ATX_","U")
               CALL  trd_flx( ztrdy, jn, "ATY_","V")
               CALL  trd_flx( ztrdz, jn, "ATZ_","W")
            ENDIF
#endif
            CALL trd_tra( kt, cdtype, jn, jptra_xad, ztrdx, pun, ptn(:,:,:,jn) )   
            CALL trd_tra( kt, cdtype, jn, jptra_yad, ztrdy, pvn, ptn(:,:,:,jn) )  
   ...
   SUBROUTINE tra_adv_tvd_zts_bio ( kt, kit000, cdtype, p2dt, pun, pvn, pwn0,      &
      &                                       ptb, ptn, pta, kjpt, jnzts, biosink )
      !!----------------------------------------------------------------------
      !!                  ***  ROUTINE tra_adv_tvd_zts  ***
      !! 
      !! **  Purpose :   Compute the now trend due to total advection of 
      ...
      INTEGER                              , INTENT(in   ) ::   kjpt            ! number of tracers
      REAL(wp), DIMENSION(        jpk     ), INTENT(in   ) ::   p2dt            ! vertical profile of tracer time-step
      REAL(wp), DIMENSION(jpi,jpj,jpk     ), INTENT(in   ) ::   pun, pvn, pwn0   ! 3 ocean velocity components
      REAL(wp), DIMENSION(jpi,jpj,jpk,kjpt), INTENT(in   ) ::   ptb, ptn        ! before and now tracer fields
      REAL(wp), DIMENSION(jpi,jpj,jpk,kjpt), INTENT(inout) ::   pta             ! tracer trend 
      INTEGER                              , INTENT(in   ) ::   jnzts           ! number of sub-timesteps for vertical advection
      REAL(wp), DIMENSION(jpi,jpj,jpk,kjpt), INTENT(in)    ::   biosink          ! tracer trend 
      !
      REAL(wp), DIMENSION( jpk )                           ::   zts             ! length of sub-timestep for vertical advection
      REAL(wp), DIMENSION( jpk )                           ::   zr_p2dt         ! reciprocal of tracer timestep
      INTEGER  ::   ji, jj, jk, jl, jn       ! dummy loop indices  
      ...
      REAL(wp) ::   zfm_ui, zfm_vj, zfm_wk   !   -      -
      REAL(wp), POINTER, DIMENSION(:,:  ) :: zwx_sav , zwy_sav
      REAL(wp), POINTER, DIMENSION(:,:,:) :: zwi, zwz, zhdiv, zwz_sav, zwzts, pwn
      REAL(wp), POINTER, DIMENSION(:,:,:) :: ztrdx, ztrdy, ztrdz
      REAL(wp), POINTER, DIMENSION(:,:,:,:) :: ztrs
      !!----------------------------------------------------------------------
      !
      ...
      !
      CALL wrk_alloc( jpi, jpj, zwx_sav, zwy_sav )
      CALL wrk_alloc( jpi, jpj, jpk, zwi, zwz , zhdiv, zwz_sav, zwzts, pwn )
      CALL wrk_alloc( jpi, jpj, jpk, 3, ztrs ) ! EO: this is not kjpt, it is before/now/after
      !
      IF( kt == kit000 )  THEN
         IF(lwp) WRITE(numout,*)
         IF(lwp) WRITE(numout,*) 'tra_adv_tvd_zts : TVD ZTS advection scheme on ', cdtype
      ...
      DO jn = 1, kjpt                                            ! tracer loop
         !                                                       ! ===========
         pwn = pwn0(:,:,:) + biosink(:,:,:,jn) * wmask(:, :, :)    ! just ensure w is zero at the bottom as required by TVD
         ! 1. Bottom value : flux set to zero
         ! ----------------------------------
         zwx(:,:,jpk) = 0._wp   ;    zwz(:,:,jpk) = 0._wp
         zwy(:,:,jpk) = 0._wp   ;    zwi(:,:,jpk) = 0._wp
         ...
         ! for variables with bio sinking, set last flux above bottom to upwind value 
         ! ( due to concentration accumulation in bottom cell )
         IF ( jn == jpdsi .OR. jn == jppon .OR. jn == jpdia) THEN
            DO jj = 1, jpj
               DO ji = 1, jpi
                  zwz(ji,jj,mbkt(ji,jj)) = 0.0_wp
               END DO
            END DO
         END IF
#endif
      ...
      END DO
      !
                   CALL wrk_dealloc( jpi, jpj, jpk, zwi, zwz, zhdiv, zwz_sav, zwzts, pwn )
      ...
      IF( nn_timing == 1 )  CALL timing_stop('tra_adv_tvd_zts')
      !
   END SUBROUTINE tra_adv_tvd_zts_bio
